...
{
	diag_log text "[MadinAI_fnc_AiGroupLoop] group is grpNull, exit sctipt"
};
private _leader = leader _group;
if (!local _leader)exitWith {};

private _aiEnabled = _group getVariable ["MadinAI",true];
...
if (_PTSD > 0) then{
	...
	if (MAI_allowFlee && {_PTSD >= MAI_PTSDflee}) then {
		[_group] call MadinAI_fnc_AiGroupFlee;
	};
};
_group setVariable ["MAI_lastLoop",time];

if (behaviour _leader == "COMBAT") exitWith{
	[_group] call MadinAi_fnc_AiGroupCombat;
};

/*
if (behaviour _leader == "AWARE") exitWith{
	[_group] call MadinAi_fnc_AiGroupAware;
};
*/